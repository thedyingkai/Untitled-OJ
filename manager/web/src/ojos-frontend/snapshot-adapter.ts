...
export interface ContributionSnapshotObserverV1 {
  (snapshot: unknown, adapted: FrontendContributionSnapshotV1): void;
}
...
export function createContributionSnapshotFetcher(
  ...
  onValidatedSnapshot?: ContributionSnapshotObserverV1,
  ...
  return async (requestedTarget, signal) => {
    ...
    onValidatedSnapshot?.(raw, adapted);
    return adapted;
  };
}
