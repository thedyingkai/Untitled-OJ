...
if [[ -n "$target_operation" ]]; then
  ...
  response="$(curl_json POST "$base_url/operations/$target_operation/rollback" '{}')"
  ...
  path="$path/rollback"
  body='{}'
  if [[ -n "${OJOS_ROLLBACK_TARGET_OPERATION_ID:-}" ]]; then
    body="$(jq -cn --arg target "$OJOS_ROLLBACK_TARGET_OPERATION_ID" '{fields:{target_operation_id:$target}}')"
  fi
