...
Operation 工作台同样由 core 统一选择 Store。未设置 `ORCHESTRATOR_DATABASE_URL` 时，plan、confirm、apply、rollback 使用 `MemoryOrchestratorStore` 进行本地演示；设置该变量时，plan/update 会把 `PLANNED` Operation 写入 `PgOrchestratorStore`，confirm 会写入 `AWAITING_CONFIRMATION`，apply/rollback 会先把当前 Service、Set、Endpoint、Link、Topology 和 Operation seed 到 `PgOrchestratorStore`，再由 `OperationExecutor` 写入 operation 状态、step log、result 和 rollback 记录。
