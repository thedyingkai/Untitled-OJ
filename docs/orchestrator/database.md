...
OJOS uses distributed service-owned databases. `ORCHESTRATOR_DATABASE_URL` points to the Orchestrator DB. Business services use their own database URLs, such as `AUTH_DATABASE_URL`, `PROBLEM_DATABASE_URL`, `JUDGE_DATABASE_URL`, and `USER_DATABASE_URL`; they must not share one OJ business database. Business services must not write the Orchestrator DB directly, and the Orchestrator must not write service-owned business tables.
...
`services/orchestrator/migrations/000001_orchestrator_schema.up.sql` is the formal schema initializer. It must not recreate retired machine, device, installer, service installation, or runtime-manager tables.
