...
当前源码的仓库外副本已通过 6 个服务的 524 项顶层 Go 回归，其中包括 6 项新增的启动失败资源回收检查。真实 TLS PostgreSQL 和带认证 Redis 用于连接回收、Auth 持久化、Judge 任务与事件投影等场景；85 个声明的职责拆分和健康检查入口另做了保真核对。

Windows 未提供符号链接创建能力的题包拒绝场景，以及未配置 MinIO 的对象存储场景各跳过一项，不计作通过。编译和构造回滚不替代实际部署的信号退出、Gateway/Auth 链路与 OIDC 验收；这些属于[重构计划](refactoring-plan.md)中的 J 批。验证代码、服务夹具和运行报告均不在本仓库。
