# Orchestrator v1.0 未完成的上线证据

当前未完成项不再是连接池、持久化、Node 生命周期、Store、Topology、Desktop、Web/TUI 或真实升级/恢复功能实现；这些能力已经进入 v1 代码和自动化契约。这里仅记录尚不能由仓库内短时测试自行证明的发布门禁。

在下列两项完成前，`1.0.0` 只能视为源码版本和发布候选，不能标记为 GA。
