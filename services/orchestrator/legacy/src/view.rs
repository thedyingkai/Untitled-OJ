...
pub struct OrchestratorView {
    ...
    pub deployments: Vec<DeploymentViewRow>,
...
/// 某个 Service 在某台主机上的精确部署。Web、TUI 和 HTTP API 共用这一投影，
/// 避免把仓库里的 Service manifest 误当成已经安装的实例。
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct DeploymentViewRow {
...
pub fn load_orchestrator_view_from_store<S: OrchestratorStore>(
    ...
    Ok(OrchestratorView {
        schemas,
        deployments: deployment_rows(store, &services, &endpoints)?,
...
fn deployment_rows<S: OrchestratorStore>(
    store: &S,
    services: &[ServiceManifest],
    endpoints: &[Endpoint],
) -> Result<Vec<DeploymentViewRow>> {
    let services = services
        .iter()
        .map(|service| (service.id.as_str(), service))
        .collect::<HashMap<_, _>>();
    let releases = store.service_releases()?;
    let mut deployments = store.host_services()?;
    ...
    deployments
        .into_iter()
        .map(|deployment| {
            let service = services.get(deployment.service_name.as_str()).copied();
            ...
            let runtime = releases
                ...
                .map(str::to_string)
                .or_else(|| service.map(|service| format!("{:?}", service.runtime.mode)))
                .unwrap_or_else(|| "unknown".to_string());
            Ok(DeploymentViewRow {
                ...
                protocol: primary_endpoint
                    ...
                    .unwrap_or_default(),
                health_path: primary_endpoint
                    .map(|endpoint| endpoint.health_path.clone())
                    .unwrap_or_default(),
                endpoint_health: primary_endpoint
