...
fn diagnostic_markdown(report: &DiagnosticReport) -> String {
    ...
    if let Some(groups) = report.data.get("service_name_endpoint_groups_summary") {
        lines.push(format!(
            "- service-name endpoint groups: {}",
            json_count(groups).unwrap_or_default()
        ));
    }
...
fn service_endpoint_groups(endpoints: &[crate::Endpoint]) -> BTreeMap<String, Vec<String>> {
