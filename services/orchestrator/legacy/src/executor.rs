...
impl LocalProcessDriver {
    ...
    fn stop_local_process(&self, request: &DriverRequest) -> Result<DriverResult> {
        ...
        match stop_process(pid, &command)? {
            ...
            ProcessStopResult::Failed(message) => Ok(DriverResult {
                action: request.action.clone(),
    ...
    pub(crate) fn has_pid_file(&self, service_id: &str) -> Result<bool> {
        self.pid_file(service_id)?.try_exists().map_err(io_error)
    }
}

enum ProcessStopResult {
    Stopped,
    Failed(String),
}

#[cfg(windows)]
fn stop_process(pid: u32, _command: &[String]) -> Result<ProcessStopResult> {
    ...
    // `taskkill` depends on Windows management/RPC services and is not
    // reliable in restricted service hosts. Open exactly the persisted root
    // PID, terminate it through the kernel API, and wait for the handle to be
    // signalled before reporting success.
    let handle = unsafe { OpenProcess(PROCESS_TERMINATE | SYNCHRONIZE_ACCESS, 0, pid) };
    if handle.is_null() {
        let error = std::io::Error::last_os_error();
        if error.raw_os_error() == Some(ERROR_INVALID_PARAMETER as i32) {
            return Ok(ProcessStopResult::Stopped);
        }
        return Ok(ProcessStopResult::Failed(format!(
            "local process service_stop could not open pid {pid}: {error}"
        )));
    }
    ...
    let detail = termination_error
        ...
        .unwrap_or_else(|| format!("process did not exit within 5000 ms (wait status {wait})"));
    Ok(ProcessStopResult::Failed(format!(
        "local process service_stop could not terminate pid {pid}: {detail}"
    )))
}

#[cfg(not(windows))]
fn stop_process(_pid: u32, command: &[String]) -> Result<ProcessStopResult> {
    let output = Command::new(&command[0]).args(&command[1..]).output();
    match output {
        Ok(output) if output.status.success() || process_already_stopped(&output) => {
            Ok(ProcessStopResult::Stopped)
        }
        Ok(output) => Ok(ProcessStopResult::Failed(driver_output_message(&output)?)),
        Err(err) => Ok(ProcessStopResult::Failed(format!(
            "local process service_stop failed to run stop command: {err}"
        ))),
    }
}
...
fn stop_process_command(pid: u32) -> Result<Vec<String>> {
    Ok(vec!["Win32.TerminateProcess".to_string(), pid.to_string()])
}
...
fn io_error(error: std::io::Error) -> OrchestratorError {
    OrchestratorError::Dependency(format!("runtime I/O failed: {}", error.kind()))
}
