...
"#;
    ...
    fn import_from_yaml_synthesizes_valid_manifest() {
        ...
        assert!(!crate::release_supports_link_probe_v1(&import.release));
        assert!(import.service.provides.capabilities.is_empty());
    }

    #[test]
    fn signed_link_probe_api_projects_an_explicit_service_capability() {
        let release = format!(
        ...
        let malformed = release.replace("methods: [GET]", "methods: [POST]");
        let error = external_release_import_from_yaml(
            &malformed,
            "https://example.com/sample-module.zip",
            "sha256:abc",
        )
        ...
        assert!(
            error
                .to_string()
                .contains("must declare HTTP default port GET /probe")
        );
    }
