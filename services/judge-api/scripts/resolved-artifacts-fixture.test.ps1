...
$first = Join-Path ([IO.Path]::GetTempPath()) ("ojos-judge-artifacts-{0}.json" -f [Guid]::NewGuid())
$second = Join-Path ([IO.Path]::GetTempPath()) ("ojos-judge-artifacts-{0}.json" -f [Guid]::NewGuid())
try {
    ...
    foreach ($slot in @('judge-runtime', 'judge-migration-v1')) {
    ...
    foreach ($slot in @('judge-user-frontend', 'judge-admin-frontend')) {
    ...
    foreach ($slot in @('contract', 'openapi.judge.submission.create', 'openapi.judge.submission.result', 'openapi.judge.worker.status', 'openapi.judge.queue.status', 'openapi.judge.worker.control', 'event.io.ojos.problem.snapshot.v1.v1.schema', 'event.io.ojos.problem.deleted.v1.v1.schema', 'sbom', 'provenance', 'events')) {
