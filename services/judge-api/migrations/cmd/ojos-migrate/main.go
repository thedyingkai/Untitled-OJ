...
func main() {
	if err := run(os.Args[1:]); err != nil {
		log.Printf("judge migration failed: %v", err)
		os.Exit(1)
	}
}
...
func run(arguments []string) error {
	...
	if err != nil {
		return errors.New("connect to claimed judge PostgreSQL database")
	}
	defer connection.Close(context.Background())
	if _, err := connection.Exec(ctx, `SELECT pg_advisory_lock(hashtext('ojos:judge-api:migration'))`); err != nil {
		return err
	}
	defer connection.Exec(context.Background(), `SELECT pg_advisory_unlock(hashtext('ojos:judge-api:migration'))`)
...
func databaseDSN() (string, error) {
	path := strings.TrimSpace(os.Getenv("OJOS_RESOURCE_SUBMISSIONS_OUTPUT_FILE"))
	if path == "" {
		path = strings.TrimSpace(os.Getenv("OJOS_RESOURCE_OUTPUT_FILE"))
	}
	if path == "" {
		path = "/run/ojos/resources/submissions/dsn"
	}
