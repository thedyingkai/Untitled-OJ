...
const (
	...
	storageDeleteBinding = "storage_delete"
)
...
func NewBoundObjectStore(bucket string) (*BoundObjectStore, error) {
	...
	for _, name := range []string{storageHeadBinding, storageDeleteBinding} {
		if _, err := value.Binding(name); err != nil {
	...
	return &BoundObjectStore{Context: *value, Client: client, Bucket: bucket}, nil
}
...
func (s BoundObjectStore) Inspect(ctx context.Context, intent Intent) (Object, bool, error) {
	...
	resp, err := s.Context.Do(ctx, s.Client, storageHeadBinding, http.MethodHead, path, nil)
	if err != nil {
		return Object{}, false, err
	}
...
func (s BoundObjectStore) DeleteIfMatches(ctx context.Context, intent Intent) error {
	...
	resp, err := s.Context.DoWithOptions(ctx, s.Client, storageDeleteBinding, http.MethodDelete, path, nil, servicecontext.RequestOptions{Headers: headers, ContentLength: 0})
	if err != nil {
		return err
	}
	defer resp.Body.Close()
...
func (s BoundObjectStore) DeleteBindingTimeout() (time.Duration, error) {
	binding, err := s.Context.Binding(storageDeleteBinding)
	if err != nil {
		return 0, fmt.Errorf("artifact GC: %w", err)
	}
	timeout := time.Duration(binding.TimeoutMS) * time.Millisecond
	if timeout <= 0 {
		return 0, errors.New("artifact GC storage_delete binding timeout is invalid")
	}
	return timeout, nil
}
