...
$rawSourceBySlot = @{
    ...
    'openapi.problem.problem.read' = Join-Path $serviceRoot 'api\problem-read.openapi.yaml'
    'openapi.problem.problem.manage' = Join-Path $serviceRoot 'api\problem-manage.openapi.yaml'
    'openapi.problem.testdata.read' = Join-Path $serviceRoot 'api\testdata-read.openapi.yaml'
    'openapi.problem.testdata.write' = Join-Path $serviceRoot 'api\testdata-write.openapi.yaml'
    'openapi.problem.artifact-gc.manage' = Join-Path $serviceRoot 'api\artifact-gc.openapi.yaml'
    ...
    'problem-user-frontend' = Join-Path $serviceRoot 'frontend\user\bundle.js'
    'problem-admin-frontend' = Join-Path $serviceRoot 'frontend\admin\bundle.js'
}
$fixturePayloadBySlot = [ordered]@{
    'problem-runtime' = [Text.Encoding]::UTF8.GetBytes('fixture:problem-runtime:0.1.0')
    'problem-migration-v1' = [Text.Encoding]::UTF8.GetBytes('fixture:problem-migration-v1:0.1.0')
}
...
foreach ($requirement in $buildInput.artifactRequirements) {
    ...
    if ($requirement.slot -in @('problem-user-frontend', 'problem-admin-frontend')) {
