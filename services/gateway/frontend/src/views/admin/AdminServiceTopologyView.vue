...
import { getOrchestratorSnapshot, listEndpointGroups } from '../../api/services'
...
const endpointGroupColumns: DataTableColumns<EndpointGroupItem> = [
  { title: 'Service', key: 'service_name', width: 220 },
  { title: 'Selector', key: 'selector', width: 220 },
  { title: 'Endpoint Count', key: 'endpoint_count', width: 140 },
  { title: 'Endpoints', key: 'endpoints', render: (row) => row.endpoints.join(', ') },
]
...
function topologyFromSnapshot(
  endpointGroups: EndpointGroupItem[],
...
async function load(silent = false): Promise<void> {
  ...
  error.value = null
  try {
    const [groupResp, snapshot] = await Promise.all([listEndpointGroups(), getOrchestratorSnapshot()])
    topology.value = topologyFromSnapshot(groupResp.endpoint_groups, snapshot)
  } catch (err) {
  ...
  <PageCard title="Service Topology">
    ...
    <template v-else>
      ...
      <NTabs v-else type="line" animated>
        <NTabPane name="endpoint-groups" tab="Endpoint Groups">
          <EmptyView v-if="!topology?.endpoint_groups.length" description="暂无 Endpoint Group" />
          <NDataTable v-else :columns="endpointGroupColumns" :data="topology.endpoint_groups" :pagination="{ pageSize: 10 }" :bordered="false" />
        </NTabPane>
