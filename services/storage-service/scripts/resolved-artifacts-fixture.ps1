...
$rawSourceBySlot = @{
    ...
    'openapi.storage.object.put' = Join-Path $serviceRoot 'api\object-put.openapi.yaml'
    'openapi.storage.object.get' = Join-Path $serviceRoot 'api\object-get.openapi.yaml'
    'openapi.storage.object.head' = Join-Path $serviceRoot 'api\object-head.openapi.yaml'
    'openapi.storage.object.delete' = Join-Path $serviceRoot 'api\object-delete.openapi.yaml'
}

$runtimePayload = [Text.Encoding]::UTF8.GetBytes('fixture:storage-runtime:0.1.0')
$runtimeSHA = [Security.Cryptography.SHA256]::Create()
try { $runtimeHash = -join ($runtimeSHA.ComputeHash($runtimePayload) | ForEach-Object { $_.ToString('x2') }) } finally { $runtimeSHA.Dispose() }
$artifacts = [ordered]@{
    'storage-runtime' = [ordered]@{
        mediaType = 'application/vnd.oci.image.manifest.v1+json'
        digest = "sha256:$runtimeHash"
        size = $runtimePayload.Length
        reference = "example.invalid/ojos/storage-runtime@sha256:$runtimeHash"
    }
}
...
foreach ($requirement in $buildInput.artifactRequirements) {
    ...
    if ($requirement.slot -eq 'config.schema') {
        # The compiler/publisher binds JSON artifacts by canonical JSON bytes,
        # so use its declared expected digest rather than source indentation.
        if ([string]::IsNullOrWhiteSpace($requirement.expectedDigest)) { throw 'config.schema has no canonical expected digest' }
