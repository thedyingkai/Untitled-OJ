...
$first = Join-Path ([IO.Path]::GetTempPath()) ("ojos-user-artifacts-{0}.json" -f [Guid]::NewGuid())
$second = Join-Path ([IO.Path]::GetTempPath()) ("ojos-user-artifacts-{0}.json" -f [Guid]::NewGuid())
try {
    ...
    foreach ($slot in @('user-runtime', 'user-migration-v1')) {
        if ($document.artifacts.$slot.reference -notmatch '^example\.invalid/ojos/.+@sha256:[a-f0-9]{64}$') {
            throw "fixture OCI slot $slot is not digest-pinned"
        }
    }
    foreach ($slot in @('user-frontend', 'user-admin-frontend')) {
        if ($document.artifacts.$slot.reference -notmatch '^https://fixture\.invalid/__ojos/extensions/[a-f0-9]{64}/bundle\.js$') {
            throw "fixture frontend slot $slot is not content-addressed"
        }
    }
    foreach ($slot in @('contract', 'openapi.user-service.api', 'sbom', 'provenance', 'events')) {
