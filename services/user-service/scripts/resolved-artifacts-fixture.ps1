...
$rawSourceBySlot = @{
    ...
    'openapi.user-service.api' = Join-Path $serviceRoot 'api\openapi.yaml'
    'user-frontend' = Join-Path $serviceRoot 'frontend\user\bundle.js'
    'user-admin-frontend' = Join-Path $serviceRoot 'frontend\admin\bundle.js'
}
$fixturePayloadBySlot = [ordered]@{
    'user-runtime' = [Text.Encoding]::UTF8.GetBytes('fixture:user-runtime:0.1.0')
    'user-migration-v1' = [Text.Encoding]::UTF8.GetBytes('fixture:user-migration-v1:0.1.0')
}
...
foreach ($requirement in $buildInput.artifactRequirements) {
    ...
    if ($requirement.slot -in @('user-frontend', 'user-admin-frontend')) {
