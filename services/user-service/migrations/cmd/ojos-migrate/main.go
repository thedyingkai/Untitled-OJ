...
const defaultResourceOutput = "/run/ojos/resources/profiles/dsn"

func main() {
	if err := run(os.Args[1:]); err != nil {
		log.Printf("user-service migration failed: %v", err)
		os.Exit(1)
	}
}
...
func run(arguments []string) error {
	...
	if _, err := connection.Exec(ctx, `SELECT pg_advisory_lock(hashtext('ojos:user-service:migration'))`); err != nil {
		return errors.New("acquire user-service migration lock")
	}
	defer connection.Exec(context.Background(), `SELECT pg_advisory_unlock(hashtext('ojos:user-service:migration'))`)
...
func databaseDSN() (string, error) {
	path := strings.TrimSpace(os.Getenv("OJOS_RESOURCE_PROFILES_OUTPUT_FILE"))
	if path == "" {
