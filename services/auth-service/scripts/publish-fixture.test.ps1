...
$scratch = Join-Path ([IO.Path]::GetTempPath()) ("ojos-auth-publish-{0}" -f [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    try {
        & cargo run -p ojos-service -- service publish `
            ...
            --key-id auth-fixture-test-key `
            --catalog-id auth-fixture-test `
    ...
    $metadataPath = Join-Path $catalogOutput 'metadata\auth-service-0.1.0.release.json'
    $publishedLockPath = Join-Path $catalogOutput 'metadata\auth-service-0.1.0.release.lock.json'
    ...
    if ($catalog.signatures[0].key_id -ne 'auth-fixture-test-key' -or $catalog.signatures[0].algorithm -ne 'Ed25519') { throw 'published Catalog signature identity is invalid' }
    $trust = Get-Content -Raw -LiteralPath $trustPath | ConvertFrom-Json
    if ([string]::IsNullOrWhiteSpace($trust.'auth-fixture-test-key')) { throw 'published trust document is missing the test verification key' }
    ...
    if ((Get-FileHash -Algorithm SHA256 -LiteralPath $releaseLock).Hash -ne (Get-FileHash -Algorithm SHA256 -LiteralPath $publishedLockPath).Hash) { throw 'published Catalog release lock differs from sealed release lock' }
    $metadata = Get-Content -Raw -LiteralPath $metadataPath | ConvertFrom-Json
    $ownedPermissionKeys = @($metadata.platform.contribution.permissionDefinitions | ForEach-Object { $_.key })
    if ($ownedPermissionKeys -contains 'system.admin') { throw 'external system.admin reference was projected as an Auth-owned permission definition' }
    if ($ownedPermissionKeys -contains 'auth.admin') { throw 'obsolete auth.admin compatibility permission was published' }
    if ($ownedPermissionKeys -notcontains 'auth.control' -or $ownedPermissionKeys -notcontains 'auth.permission.check') { throw 'Auth-owned permission definitions are incomplete' }
    $adminRoutes = @($metadata.platform.contribution.operationRoutes | Where-Object { $_.audience -eq 'admin' })
    if ($adminRoutes.Count -eq 0 -or @($adminRoutes | Where-Object { $_.permission -ne 'system.admin' }).Count -ne 0) { throw 'Auth admin routes do not retain the system.admin external permission reference' }
} finally {
    $resolvedScratch = [IO.Path]::GetFullPath($scratch)
    $tempRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath())
    if ($resolvedScratch.StartsWith($tempRoot, [StringComparison]::OrdinalIgnoreCase) -and (Split-Path -Leaf $resolvedScratch).StartsWith('ojos-auth-publish-')) {
