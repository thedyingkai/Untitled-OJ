...
$first = Join-Path ([IO.Path]::GetTempPath()) ("ojos-auth-artifacts-{0}.json" -f [Guid]::NewGuid())
$second = Join-Path ([IO.Path]::GetTempPath()) ("ojos-auth-artifacts-{0}.json" -f [Guid]::NewGuid())
try {
    ...
    foreach ($slot in @('auth-runtime', 'auth-migration-v1')) {
    ...
    foreach ($slot in @('contract', 'openapi.auth-service.api', 'openapi.auth.user.permission.check', 'openapi.auth.control.v1', 'sbom', 'provenance', 'events')) {
