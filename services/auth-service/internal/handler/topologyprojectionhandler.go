...
func topologyProjectionHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodGet:
			...
			if document == nil {
				writeTopologyJSON(w, http.StatusOK, shared.AbsentStatus("auth", path.TopologyID))
				return
			}
		...
		case http.MethodPut, http.MethodDelete:
			...
			if err := request.Validate("auth", path.TopologyID); err != nil {
...
func writeTopologyProblem(w http.ResponseWriter, status int, code, detail string) {
	...
	w.WriteHeader(status)
	_ = json.NewEncoder(w).Encode(map[string]any{"type": "urn:ojos:problem:" + strings.ToLower(code), "title": code, "status": status, "detail": detail})
}
