use orchestrator_core::{
    ...
    ServiceSetLink, ServiceSetOperations, ServiceSetService, SourceDecl, TopologySnapshot,
    build_diagnostic_report, build_topology, plan_operation,
};
...
fn pg_store_persists_core_objects_and_reads_them_back() {
    ...
    let problem_api = service_manifest(format!("problem-api-{suffix}"), 18083);
    store
        .upsert_service(gateway.clone())
        .expect("upsert gateway service");
    store
        .upsert_service(problem_api.clone())
        .expect("upsert problem-api service");

    let set = service_set(&suffix, &gateway.id, &problem_api.id);
    store.upsert_set(set.clone()).expect("upsert service set");

    let gateway_endpoint = Endpoint {
        endpoint: format!("127.0.0.1:{}", 18080 + port_offset(&suffix)),
    ...
    let problem_endpoint = Endpoint {
        endpoint: format!("127.0.0.1:{}", 18083 + port_offset(&suffix)),
        service_id: problem_api.id.clone(),
        protocol: "http".to_string(),
        health_path: "/health".to_string(),
        health: "unknown".to_string(),
        reachable: false,
    ...
    let topology = build_topology(
        ...
        vec![gateway.id.clone(), problem_api.id.clone()],
        vec![set.id.clone()],
        vec![
            store
...
fn service_set(suffix: &str, gateway_id: &str, problem_id: &str) -> ServiceSet {
    ServiceSet {
        schema_version: 1,
        id: format!("set-{suffix}"),
        name: "Pg store integration set".to_string(),
        description: "Integration set".to_string(),
        scenario: json!({}),
        services: vec![
            ServiceSetService::Id(gateway_id.to_string()),
            ServiceSetService::Id(problem_id.to_string()),
        ],
        default_endpoints: vec![
            ServiceSetEndpoint {
                service: gateway_id.to_string(),
                port: 18080,
                protocol: "http".to_string(),
                expose: true,
            },
            ServiceSetEndpoint {
                service: problem_id.to_string(),
                port: 18083,
                protocol: "http".to_string(),
                expose: false,
            },
        ],
        default_links: vec![ServiceSetLink {
            from: gateway_id.to_string(),
            to: problem_id.to_string(),
            protocol: "http".to_string(),
            auth_mode: "internal".to_string(),
            scope: "api".to_string(),
            required: true,
        ...
        policies: json!({}),
        operations: ServiceSetOperations {
            install_order: vec![gateway_id.to_string(), problem_id.to_string()],
            start_order: vec![gateway_id.to_string(), problem_id.to_string()],
            stop_order: Vec::new(),
        },
        notes: String::new(),
    }
